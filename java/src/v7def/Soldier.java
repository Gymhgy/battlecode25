...
public class Soldier {
    ...
    static void run(RobotController rc) throws GameActionException {
        ...
        if (curRuin != null) {
            if (curRuinType != UnitType.LEVEL_ONE_MONEY_TOWER) {
                ...
                for (MapLocation loc : rc.getAllLocationsWithinRadiusSquared(curRuin.getMapLocation(), 2)) {
                    if (rc.canSenseLocation(loc) && rc.senseMapInfo(loc).getMark() != PaintType.EMPTY) {
                        alrMarked = true;
                        break;
                    }
                    if (rc.canMark(loc)) {
    ...
    static void attackTower(RobotController rc) throws GameActionException {
        ...
        if (rc.canSenseLocation(closestEnemyTower) && !rc.isLocationOccupied(closestEnemyTower)) {
            //if (rc.canMark(rc.getLocation().add(rc.getLocation().directionTo(closestEnemyTower)))) {
                //rc.mark(rc.getLocation().add(rc.getLocation().directionTo(closestEnemyTower)), false);
            //}
            closestEnemyTower = null;
        }
    }
    ...
    static boolean ruinCheckOld(RobotController rc, MapInfo ruin) throws GameActionException {
        ...
        for (MapLocation loc : surroundingLocations) {
            ...
            if (rc.senseMapInfo(loc).getMark() == PaintType.ALLY_SECONDARY) {
                ...
                curRuinType = UnitType.LEVEL_ONE_DEFENSE_TOWER;
            }
    ...
    static void tryBuild(RobotController rc) throws GameActionException {
        ...
        int[][] tower = curRuinType == UnitType.LEVEL_ONE_MONEY_TOWER ? moneyTowerPattern :
                curRuinType == UnitType.LEVEL_ONE_DEFENSE_TOWER ? defenseTowerPattern : paintTowerPattern;
