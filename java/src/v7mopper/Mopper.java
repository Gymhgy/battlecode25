...
public class Mopper {
    ...
    static void harass(RobotController rc) throws GameActionException {
        ...
        for (RobotInfo enemy : enemies) {
            if (enemy.getType() == UnitType.MOPPER || enemy.getType().isTowerType() || enemy.paintAmount == 0) continue;
            if (target == null) {
                target = enemy;
            } else {
                if (rc.getLocation().distanceSquaredTo(enemy.location) <= 8 && enemy.paintAmount < 15) {
                    target = enemy;
                    break;
                } else if (rc.getLocation().distanceSquaredTo(enemy.location) < rc.getLocation().distanceSquaredTo(target.location)) {
                    target = enemy;
                } else if (rc.getLocation().distanceSquaredTo(enemy.location) == rc.getLocation().distanceSquaredTo(target.location)) {
                    if (enemy.paintAmount < target.paintAmount) {
                        target = enemy;
                    }
                }
            }
        ...
        if (target != null) {
            ...
            //if(loc.directionTo(target))
            targetId = target.ID;
            if (rc.isActionReady())
                performAttack(rc);
            Pathfinding.moveToward(rc, target.getLocation());
            if (rc.isActionReady())
                performAttack(rc);
        }
    }
    ...
    static void goTowardsEnemyPaint(RobotController rc) throws GameActionException {
        ...
        if (target == null || !rc.canSenseLocation(target) || !rc.senseMapInfo(target).getPaint().isEnemy()) {
    ...
    static void performAttack(RobotController rc) throws GameActionException {
        ...
        RobotInfo hitEnemy = null;
        int minPaint = Integer.MAX_VALUE;

        for (RobotInfo enemy : enemies) {
            // Check if the enemy is the one with the least paint
            if (rc.senseMapInfo(enemy.getLocation()).getPaint().isEnemy()) {
                if(enemy.paintAmount == 0) continue;
                int enemyPaint = enemy.paintAmount;
                if (hitEnemy == null || enemyPaint < minPaint) {
                    hitEnemy = enemy;
                    minPaint = enemyPaint;
                }
            }
...
// After the loop, if hitEnemy is set, get the location of the enemy with the least paint
        if (hitEnemy != null) {
            hitLoc = hitEnemy.getLocation();
        }
