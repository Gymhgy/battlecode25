...
public class Soldier {
    ...
    private static void retreat(RobotController rc) throws GameActionException {
        ...
        if (paintTowers.size() > 0 && (paintTowers.closest(loc).isWithinDistanceSquared(loc, 35))) {
            ...
            soldierMicro.doMicro(closestEnemyTower, true);
        }
    ...
    private static boolean canBeat(RobotController rc) throws GameActionException {
        ...
        for (var ri : allies) {
            if (ri.getType() == UnitType.SOLDIER) {
                if (ri.getLocation().isWithinDistanceSquared(closestEnemyTower, 12) && ri.getPaintAmount() > 100) {
                    mult++;
                }
            }
        }
    ...
    // Soldeirs hdeal 50 damage to towers with Action cooldown of once/2 turns -> /40
