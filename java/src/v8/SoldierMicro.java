...
public class SoldierMicro {
    ...
    boolean doMicro(MapLocation target, boolean moveAway){
        try {
            ...
            for (int i = 0; i < 8; ++i) {
                if (microInfo[i].isBetter(bestMicro, moveAway)) bestMicro = microInfo[i];
            }
    ...
    class MicroInfo {
        ...
        //equal => true
        boolean isBetter(MicroInfo M, boolean moveAway){
            ...
            if (!moveAway) {
                ...
                if (distToTower < M.distToTower) return false;
                if (M.distToTower < distToTower) return true;
            }
