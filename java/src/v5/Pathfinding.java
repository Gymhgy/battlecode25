...
//                    rc.setIndicatorLine(rc.getLocation(), rc.getLocation().add(prv[pathingCnt]), 255, 0, 0);
    ...
    static boolean canPass(RobotController rc, MapLocation loc, Direction targetDir) throws GameActionException {
       ...
       // if (adj >= 6) return false;
        ...
        if (robot != null)
            return FastMath.rand256()%10 == 0;

        /*if (Refill.refilling) {
            int cardinal = 0;
            if (rc.canSenseLocation(loc.add(Direction.NORTH)) && rc.sensePassability(loc.add(Direction.NORTH))) cardinal++;
            if (rc.canSenseLocation(loc.add(Direction.WEST)) && rc.sensePassability(loc.add(Direction.WEST))) cardinal++;
            if (rc.canSenseLocation(loc.add(Direction.SOUTH)) && rc.sensePassability(loc.add(Direction.SOUTH))) cardinal++;
            if (rc.canSenseLocation(loc.add(Direction.EAST)) && rc.sensePassability(loc.add(Direction.EAST))) cardinal++;
