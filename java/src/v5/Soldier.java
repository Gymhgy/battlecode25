        ...
        if (curRuin == null && rc.getNumberTowers() < 25) {
            for (MapInfo tile : nearbyTiles) {
                ...
                if (ruinCheck(rc, tile)) {
                    ...
                    for (int i = -2; i <= 2; i++) {
                        for (int j = -2; j <= 2; j++) {
                            MapLocation loc = curRuin.getMapLocation().translate(i, j);
                            if (rc.canSenseLocation(loc)) {
                                PaintType pt = rc.senseMapInfo(loc).getPaint();
                                if (pt == numToPaint(paintTowerPattern[i+2][j+2])) pTowerCnt++;
                                if (pt == numToPaint(moneyTowerPattern[i+2][j+2])) mTowerCnt++;
                            }
                        }
                    }
                    ...
                    else if (mTowerCnt >= 10) {
                        curRuinType = UnitType.LEVEL_ONE_MONEY_TOWER;
                    }
                    else if (pTowerCnt >= 10) {
                        curRuinType = UnitType.LEVEL_ONE_PAINT_TOWER;
                    } else {
